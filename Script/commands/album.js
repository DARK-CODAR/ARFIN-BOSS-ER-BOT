...
module.exports.config = {
    ...
    version: "1.0.1",
    hasPermssion: 0,
    credits: "𝙰𝚁𝙵𝙸𝙽", //file credits arfin
...
module.exports.run = async function({ api, event, args }) {
    ...
    if (args[0] === "2") {
        ...
        await api.sendMessage({ body: message2 },
            threadID,
            (error, info) => {
                if (!error) {
    ...
    if (!args[0] || args[0].toLowerCase() === "list") {
        ...
        await api.sendMessage({ body: message },
            threadID,
            (error, info) => {
                if (!error) {
                    global.client.handleReply.push({
...
module.exports.handleReply = async function({ api, event, handleReply }) {
    ...
    const adminIDs = ["61566562741771", "100079292371997", "61575629078942"];
    if (event.type === "message_reply") {
        ...
        const categories = [
            "funny", "islamic", "sad", "anime", "cartoon",
            "love", "horny", "couple", "flower", "marvel",
            "aesthetic", "sigma", "lyrics", "cat", "plus18",
        ...
        if (
            (selectedCategory === "horny" || selectedCategory === "plus18" || selectedCategory === "sex") &&
